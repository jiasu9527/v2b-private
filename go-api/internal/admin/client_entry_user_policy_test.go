...
func TestDBServiceListClientEntryUserPoliciesReturnsOneRuleWithEmails(t *testing.T) {
	...
	rows := sqlmock.NewRows([]string{"id", "entry_group_id", "entry_group_name", "enabled", "remarks", "created_at", "updated_at"}).
		AddRow(int64(3), int64(7), "香港入口", int64(1), "VIP", int64(100), int64(200))
	mock.ExpectQuery(`SELECT p.id, p.entry_group_id, g.display_name AS entry_group_name, p.enabled, p.remarks, p.created_at, p.updated_at`).
		WillReturnRows(rows)
	...
	if len(policies) != 1 || policies[0].EntryGroupName != "香港入口" {
...
func TestDBServiceSaveClientEntryUserPolicyCreatesOneRuleWithMultipleEmails(t *testing.T) {
	...
	mock.ExpectQuery(`INSERT INTO v2_client_entry_user_policy`).
		WithArgs(int64(7), int64(1), "VIP", sqlmock.AnyArg(), sqlmock.AnyArg()).
	...
	ok, err := service.SaveClientEntryUserPolicy(context.Background(), ClientEntryUserPolicySaveRequest{
		Emails: []string{" A@Example.com ", "b@example.com", "a@example.com"}, EntryGroupID: 7, Members: []ClientEntryGroupMemberSaveRequest{{ServerType: "vmess", ServerID: 11}, {ServerType: "trojan", ServerID: 12}}, Enabled: ptrInt64ForClientEntryPolicyTest(1), Remarks: "VIP",
	})
	if err != nil || !ok {
