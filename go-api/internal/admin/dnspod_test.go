...
func TestDBServiceSaveDNSPodConfigMasksSecretAndDoesNotExposeKey(t *testing.T) {
	...
	if !status.Configured || status.SecretIDMasked != "AKID12****7890" || status.Source != "config" {
...
func TestDBServiceDNSPodEnvironmentCredentialsTakePrecedence(t *testing.T) {
	...
	if !status.Configured || status.Source != "env" || status.SecretIDMasked != "ENVSEC****1234" {
...
func TestDBServiceDNSPodOperationsValidateAndDelegate(t *testing.T) {
	...
	service := &DBService{dnspodClientFactory: func(_, _ string) dnspodAPI { return fake }}
