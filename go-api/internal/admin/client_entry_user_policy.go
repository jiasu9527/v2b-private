...
func (s *DBService) ListClientEntryUserPolicies(ctx context.Context) ([]ClientEntryUserPolicyRecord, error) {
	...
	rows, err := s.db.QueryContext(ctx, `SELECT p.id, p.entry_host, p.enabled, p.remarks, p.created_at, p.updated_at
...
ORDER BY p.id DESC`)
	...
	for rows.Next() {
		...
		if err := rows.Scan(&record.ID, &record.EntryHost, &record.Enabled, &record.Remarks, &record.CreatedAt, &record.UpdatedAt); err != nil {
...
func (s *DBService) SaveClientEntryUserPolicy(ctx context.Context, req ClientEntryUserPolicySaveRequest) (bool, error) {
	...
	if req.EntryHost == "" {
		return false, errors.New("节点地址不能为空")
	}
