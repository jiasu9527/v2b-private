...
func Load() Config {
	jsonConfigPath := "../config/admin.json"
	legacyPHPConfigPath := "../config/v2board.php"
	jsonConfig := loadJSONConfigMap(jsonConfigPath)
	defaultWithdrawMethods := []string{"支付宝", "USDT", "Paypal"}

	return Config{
		AppName:                      getEnv("APP_NAME", loadConfigString(jsonConfig, legacyPHPConfigPath, "app_name", "forest-go-api")),
