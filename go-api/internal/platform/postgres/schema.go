...
CONSTRAINT uniq_v2_client_entry_user_policy_user UNIQUE (policy_id, email)
		...
		`CREATE TABLE IF NOT EXISTS v2_client_entry_user_policy_entry (
id INTEGER GENERATED BY DEFAULT AS IDENTITY NOT NULL,
policy_id INTEGER NOT NULL,
...
updated_at INTEGER NOT NULL,
PRIMARY KEY (id),
CONSTRAINT uniq_v2_client_entry_user_policy_entry UNIQUE (policy_id, entry)
)`,
	} {
	...
	if _, err := db.ExecContext(ctx, `ALTER TABLE v2_client_entry_user_policy ALTER COLUMN entry_group_id SET DEFAULT 0`); err != nil {
		return fmt.Errorf("ensure client entry user policy entry_group_id default: %w", err)
	}
	...
	for _, stmt := range []string{
		...
		`CREATE INDEX IF NOT EXISTS idx_v2_client_entry_user_policy_entry_policy ON v2_client_entry_user_policy_entry(policy_id)`,
	} {
		if _, err := db.ExecContext(ctx, stmt); err != nil {
...
ON CONFLICT DO NOTHING`); err != nil {
	...
	if _, err := db.ExecContext(ctx, `INSERT INTO v2_client_entry_user_policy_entry (policy_id, entry, sort, created_at, updated_at)
SELECT p.id, ip.ip, ip.sort, p.created_at, p.updated_at
FROM v2_client_entry_user_policy p
JOIN v2_client_entry_group_ip ip ON ip.entry_group_id = p.entry_group_id
WHERE p.entry_group_id > 0
ON CONFLICT DO NOTHING`); err != nil {
		return fmt.Errorf("backfill client entry user policy entries: %w", err)
	}
