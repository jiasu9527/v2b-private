...
func expectDNSFailoverSchema(mock sqlmock.Sqlmock) {
	for _, pattern := range []string{
		...
		`(?s)CREATE TABLE IF NOT EXISTS v2_dns_failover_log.*group_id BIGINT NOT NULL.*probe_id BIGINT DEFAULT NULL.*target_id BIGINT DEFAULT NULL.*stage varchar\(64\) NOT NULL.*level varchar\(16\) NOT NULL.*outcome varchar\(32\) NOT NULL.*message text NOT NULL DEFAULT ''.*details text NOT NULL DEFAULT '\{\}'.*created_at BIGINT NOT NULL`,
	...
	for _, index := range []string{
		...
		`idx_v2_dns_failover_log_created_id ON v2_dns_failover_log\(created_at DESC, id DESC\)`,
		`idx_v2_dns_failover_log_group_created_id ON v2_dns_failover_log\(group_id, created_at DESC, id DESC\)`,
		`idx_v2_dns_failover_log_probe_created_id ON v2_dns_failover_log\(probe_id, created_at DESC, id DESC\)`,
		`idx_v2_dns_failover_log_target_created_id ON v2_dns_failover_log\(target_id, created_at DESC, id DESC\)`,
		`idx_v2_dns_failover_log_stage_created_id ON v2_dns_failover_log\(stage, created_at DESC, id DESC\)`,
		`idx_v2_dns_failover_log_level_created_id ON v2_dns_failover_log\(level, created_at DESC, id DESC\)`,
		`idx_v2_dns_failover_log_outcome_created_id ON v2_dns_failover_log\(outcome, created_at DESC, id DESC\)`,
		`idx_v2_dns_failover_log_group_stage_outcome_created_id ON v2_dns_failover_log\(group_id, stage, outcome, created_at DESC, id DESC\)`,
...
var expectedDNSFailoverConstraints = []struct {
	...
	{"v2_dns_failover_log", "fk_v2_dns_failover_log_group", "FOREIGN KEY (group_id) REFERENCES v2_dns_failover_group(id) ON DELETE CASCADE"},
	{"v2_dns_failover_log", "fk_v2_dns_failover_log_probe", "FOREIGN KEY (probe_id) REFERENCES v2_dns_probe(id) ON DELETE SET NULL"},
	{"v2_dns_failover_log", "fk_v2_dns_failover_log_target", "FOREIGN KEY (target_id) REFERENCES v2_dns_failover_target(id) ON DELETE SET NULL"},
	{"v2_dns_failover_log", "chk_v2_dns_failover_log_stage", "CHECK (btrim(stage) <> '')"},
	{"v2_dns_failover_log", "chk_v2_dns_failover_log_level", "CHECK (level IN ('debug', 'info', 'warning', 'error'))"},
	{"v2_dns_failover_log", "chk_v2_dns_failover_log_outcome", "CHECK (btrim(outcome) <> '')"},
