...
CONSTRAINT uniq_v2_dns_probe_result_inbox_result UNIQUE (probe_id, result_id)
		...
		`CREATE TABLE IF NOT EXISTS v2_dns_failover_log (
id BIGINT GENERATED BY DEFAULT AS IDENTITY NOT NULL,
group_id BIGINT NOT NULL,
...
$dns_failover$;`, constraint.table, constraint.name, constraint.definition)
	...
	for _, stmt := range []string{
		...
		`CREATE INDEX IF NOT EXISTS idx_v2_dns_failover_log_created_id ON v2_dns_failover_log(created_at DESC, id DESC)`,
		`CREATE INDEX IF NOT EXISTS idx_v2_dns_failover_log_group_created_id ON v2_dns_failover_log(group_id, created_at DESC, id DESC)`,
		`CREATE INDEX IF NOT EXISTS idx_v2_dns_failover_log_probe_created_id ON v2_dns_failover_log(probe_id, created_at DESC, id DESC)`,
		`CREATE INDEX IF NOT EXISTS idx_v2_dns_failover_log_target_created_id ON v2_dns_failover_log(target_id, created_at DESC, id DESC)`,
		`CREATE INDEX IF NOT EXISTS idx_v2_dns_failover_log_stage_created_id ON v2_dns_failover_log(stage, created_at DESC, id DESC)`,
		`CREATE INDEX IF NOT EXISTS idx_v2_dns_failover_log_level_created_id ON v2_dns_failover_log(level, created_at DESC, id DESC)`,
		`CREATE INDEX IF NOT EXISTS idx_v2_dns_failover_log_outcome_created_id ON v2_dns_failover_log(outcome, created_at DESC, id DESC)`,
		`CREATE INDEX IF NOT EXISTS idx_v2_dns_failover_log_group_stage_outcome_created_id ON v2_dns_failover_log(group_id, stage, outcome, created_at DESC, id DESC)`,
