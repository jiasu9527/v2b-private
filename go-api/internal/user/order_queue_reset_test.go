...
func expectNoTrafficResetCandidates(mock sqlmock.Sqlmock) {
	mock.ExpectQuery(`SELECT u.id, u.plan_id, u.u, u.d, u.expired_at, u.updated_at, p.reset_traffic_method\s+FROM v2_user u\s+JOIN v2_plan p ON p.id = u.plan_id\s+WHERE u.plan_id IS NOT NULL AND u.expired_at > \$1\s+ORDER BY u.id ASC\s+LIMIT \$2`).
