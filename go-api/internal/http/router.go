...
func NewRouter(cfg config.Config, options ...Option) http.Handler {
	...
	handler := http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		switch {
		case r.URL.Path == "/api/v1/probe/install.sh":
		...
		case r.URL.Path == adminPrefix+"/server/client-entry-user-policy/split-convert":
			if handleAdminClientEntryUserPolicySplitConvert(w, r, state.session, state.admin) {
				return
			}
		...
		case r.URL.Path == adminPrefix+"/server/client-entry-user-policy/split-group-root":
			if handleAdminClientEntryUserPolicySplitGroupRoot(w, r, state.session, state.admin) {
				return
			}
		...
		case r.URL.Path == adminPrefix+"/subscribe-guard/user-search":
			if handleAdminSubscribeGuardUserSearch(w, r, cfg, state.session, state.admin) {
				return
			}
