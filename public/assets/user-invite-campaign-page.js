(function () {
    ...
    var state = {
        ...
        settings: {
            reward_amount: 1000,
            expire_hours: 48,
            invitee_try_out_transfer_gb: 0,
            invitee_try_out_hours: 0
        },
        countdownTimer: null
    };
    ...
    function getRewardAmount() {
        return getSettingNumber('reward_amount', 1000);
    }

    function getExpireHours() {
        return getSettingNumber('expire_hours', 48);
    }

    function getInviteeTryOutTransferGb() {
        return getSettingNumber('invitee_try_out_transfer_gb', 0);
    }

    function getInviteeTryOutHours() {
        return getSettingNumber('invitee_try_out_hours', 0);
    }

    function formatInviteeTryOutRule() {
        var transfer = getInviteeTryOutTransferGb();
        var hours = getInviteeTryOutHours();
        var parts = [];
        if (transfer > 0) {
            parts.push(transfer + 'GB 流量');
        }
        if (hours > 0) {
            parts.push(hours + ' 小时体验时长');
        }
        if (!parts.length) {
            return '被邀请人沿用现有试用套餐默认流量与时长';
        }
        return '被邀请人获得 ' + parts.join(' + ') + '，组别/速率/设备限制沿用现有试用套餐';
    }
            ...
            '<div class="campaign-grid">' +
                '<div class="campaign-main">' +
                    '<div class="campaign-card">' +
                        '<div class="campaign-hero">' +
                            '<div>' +
                                '<h1>邀请活动任务</h1>' +
                                '<p>邀请好友注册后，每注册 1 人累计抵扣 ' + escapeHtml(formatMoney(getRewardAmount())) + '。任务有效期 ' + escapeHtml(getExpireHours()) + ' 小时，达标后购买绑定套餐会自动抵扣。</p>' +
                            '</div>' +
                ...
                '<div class="campaign-side">' +
                    '<div class="campaign-card">' +
                        ...
                        '<div class="campaign-kv"><div class="campaign-kv-key">奖励规则</div><div class="campaign-kv-value">每邀请 1 人注册，增加 ' + escapeHtml(formatMoney(getRewardAmount())) + ' 套餐抵扣</div></div>' +
                        '<div class="campaign-kv"><div class="campaign-kv-key">任务期限</div><div class="campaign-kv-value">创建后固定 ' + escapeHtml(getExpireHours()) + ' 小时</div></div>' +
                        '<div class="campaign-kv"><div class="campaign-kv-key">试用规则</div><div class="campaign-kv-value">' + escapeHtml(formatInviteeTryOutRule()) + '</div></div>' +
            ...
            '<div class="campaign-grid">' +
                '<div class="campaign-main">' +
                    '<div class="campaign-card">' +
                        ...
                        '<div class="campaign-stats" style="margin-top:14px;">' +
                            ...
                            '<div class="campaign-stat"><span class="campaign-label">单次奖励</span><div class="campaign-value">' + escapeHtml(formatMoney(getRewardAmount())) + '</div></div>' +
                            '<div class="campaign-stat"><span class="campaign-label">任务有效期</span><div class="campaign-value">' + escapeHtml(getExpireHours()) + ' 小时</div></div>' +
                        '</div>' +
                ...
                '<div class="campaign-side">' +
                    '<div class="campaign-card">' +
                        ...
                        '<div class="campaign-kv"><div class="campaign-kv-key">试用奖励</div><div class="campaign-kv-value">' + escapeHtml(formatInviteeTryOutRule()) + '</div></div>' +
                        '<div class="campaign-kv"><div class="campaign-kv-key">抵扣增加</div><div class="campaign-kv-value">每邀请 1 人增加 ' + escapeHtml(formatMoney(getRewardAmount())) + '</div></div>' +
        ...
        root.innerHTML = '' +
            '<div class="campaign-grid">' +
                '<div class="campaign-main">' +
                    '<div class="campaign-card">' +
                        '<div class="campaign-hero">' +
                            '<div>' +
                                '<h1>邀请活动任务</h1>' +
                                '<p>管理员已暂时关闭活动任务。普通邀请返佣仍然正常，已创建的活动任务不会在这里新建。</p>' +
                            '</div>' +
                            '<span class="status-badge status-abandoned">已关闭</span>' +
                        '</div>' +
                        '<div class="campaign-note">当前只能继续使用普通邀请关系和返佣逻辑，新的活动任务入口已关闭。如需恢复，请在后台任务管理中重新开启活动开关。</div>' +
                        '<div class="campaign-actions" style="margin-top:18px;">' +
                            '<a class="btn btn-alt-secondary" href="' + escapeHtml(config.invitePath || '/#/invite') + '">返回邀请页</a>' +
                            '<a class="btn btn-alt-secondary" href="' + escapeHtml(config.dashboardPath || '/#/dashboard') + '">返回仪表盘</a>' +
                        '</div>' +
                ...
                '<div class="campaign-side">' +
                    '<div class="campaign-card">' +
                        '<h3>当前规则</h3>' +
                        '<div class="campaign-kv"><div class="campaign-kv-key">活动状态</div><div class="campaign-kv-value">管理员关闭中</div></div>' +
                        '<div class="campaign-kv"><div class="campaign-kv-key">普通邀请</div><div class="campaign-kv-value">仍按原有返佣逻辑执行</div></div>' +
                        '<div class="campaign-kv"><div class="campaign-kv-key">已有任务</div><div class="campaign-kv-value">已创建任务不受本次入口关闭影响</div></div>' +
                    '</div>' +
                '</div>' +
            '</div>';
    }
    ...
    async function loadCampaign() {
        ...
        try {
            ...
            state.enabled = responses[0].enabled !== false;
            state.settings = responses[0].settings || state.settings;
