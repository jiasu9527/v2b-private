...
class InviteCampaignService
{
    ...
    {
        return (int) config('v2board.invite_campaign_enable', 1) === 1;
    }

    public function getExpireHours(): int
    {
        return (int) config('v2board.invite_campaign_expire_hours', 48);
    }
    ...
    {
        return $this->getExpireHours() * 3600;
    }

    public function getInviteeTryOutTransferGb()
    {
        $value = config('v2board.invite_campaign_try_out_transfer_gb', 0);
        return is_numeric($value) ? $value + 0 : 0;
    }

    public function getInviteeTryOutTransferBytes(): int
    {
        $transferGb = $this->getInviteeTryOutTransferGb();
        if ($transferGb <= 0) {
            return 0;
        }

        return (int) round($transferGb * 1073741824);
    }

    public function getInviteeTryOutHours()
    {
        $value = config('v2board.invite_campaign_try_out_hours', 0);
        return is_numeric($value) ? $value + 0 : 0;
    }
    ...
    {
        if (!$this->isEnabled()) {
            abort(500, __('Invite campaign is disabled'));
        }
    ...
    public function getActiveCampaignByInviteCode(string $code): ?InviteCampaign
    {
        $campaign = $this->getCampaignByInviteCode($code);
    ...
    public function getTryOutPlan(): ?Plan
    {
        $planId = (int) config('v2board.try_out_plan_id', 0);
        if ($planId <= 0) {
            return null;
        }
    ...
    public function getDefaultInviteeTryOutTransferGb()
    {
        ...
        if (!$plan || !is_numeric($plan->transfer_enable)) {
            return 0;
        }

        return $this->normalizeNumeric($plan->transfer_enable + 0);
    }

    public function getDefaultInviteeTryOutHours()
    {
        $plan = $this->getTryOutPlan();
        if (!$plan) {
            return 0;
        ...
        $value = config('v2board.try_out_hour', 1);
        if (!is_numeric($value)) {
            return 0;
        }

        return $this->normalizeNumeric($value + 0);
    }
    ...
    public function getValidInviteCode(string $code): ?InviteCode
    {
    ...
    public function previewInviteCode(?string $code): array
    {
        $code = trim((string) $code);
        if ($code === '') {
            return $this->buildPreviewPayload($code, 'invalid');
        }

        $inviteCode = $this->getValidInviteCode($code);
        if (!$inviteCode || !$inviteCode->user_id) {
            return $this->buildPreviewPayload($code, 'invalid');
        }

        $campaign = $this->getActiveCampaignByInviteCode($inviteCode->code);
        if ($campaign) {
            return $this->buildPreviewPayload(
                $inviteCode->code,
                'campaign',
                $this->getEffectiveInviteeTryOutProfile($campaign),
                (int) $campaign->expired_at
            );
        }
        ...
        return $this->buildPreviewPayload(
            $inviteCode->code,
            'normal',
            $this->getEffectiveInviteeTryOutProfile()
        );
    }

    public function applyInviteeTryOut(User $user, ?InviteCampaign $campaign = null): void
    {
        $profile = $this->getEffectiveInviteeTryOutProfile($campaign);
        $plan = $profile['plan'];
        if (!$plan) {
            return;
        }

        $user->transfer_enable = (int) $profile['gift_transfer_bytes'];
        $user->device_limit = $plan->device_limit;
        $user->plan_id = $plan->id;
        $user->group_id = $plan->group_id;
        $user->expired_at = time() + (int) round(((float) $profile['gift_hours']) * 3600);
    ...
    private function buildPreviewPayload(string $code, string $type, ?array $profile = null, ?int $countdownExpiredAt = null): array
    {
        $profile = $profile ?? [
            'gift_transfer_gb' => 0,
            'gift_hours' => 0,
        ...
        return [
            'code' => $code,
            'type' => $type,
            'gift_transfer_gb' => $profile['gift_transfer_gb'],
            'gift_hours' => $profile['gift_hours'],
            'countdown_expired_at' => $countdownExpiredAt,
        ];
    }

    private function convertGbToBytes($value): int
    {
        if (!is_numeric($value) || $value <= 0) {
            return 0;
        }

        return (int) round(((float) $value) * 1073741824);
    }
