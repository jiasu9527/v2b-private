...
探针列表里的“公网 IP”由 API 根据连接来源和上面的转发头写入。默认会信任本机、Docker 网桥和常见私网反代来源；如果 Nginx、CDN 或负载均衡器使用公网地址连接 Go API，需要在 `.env.go` 中显式加入**最后一跳代理**的精确网段并重启服务，例如：

```env
PROBE_TRUSTED_PROXY_CIDRS=127.0.0.0/8,::1/128,203.0.113.10/32
```

不要填写 `0.0.0.0/0` 或 `::/0`，否则任意公网请求都可以伪造转发头。配置更新后，探针下一次心跳会自动刷新列表中的公网 IP，不需要重装探针。
