...
class InviteCampaignFlowTest extends TestCase
{
    ...
    public function test_user_cannot_create_invite_campaign_when_feature_disabled()
    {
        Config::set('v2board.invite_campaign_enable', 0);
        ...
        $this->assertSame(0, InviteCampaign::count());
    }

    public function test_user_can_fetch_invite_campaign_feature_status()
    {
        $user = $this->createUser();

        $this->withHeaders($this->userHeaders($user))
            ->getJson('/api/v1/user/invite/campaign/fetch')
            ->assertOk()
            ->assertJsonPath('enabled', true)
            ->assertJsonPath('settings.reward_amount', 1000)
            ->assertJsonPath('settings.expire_hours', 48)
            ->assertJsonPath('settings.invitee_try_out_transfer_gb', 0)
            ->assertJsonPath('settings.invitee_try_out_hours', 0)
    ...
    public function test_active_campaign_registration_uses_campaign_specific_trial_transfer_and_duration()
    {
        $inviter = $this->createUser();
        $inviteCode = $this->createInviteCode($inviter, [
            'code' => 'NORMAL001A',
        ...
        $response = $this->withHeaders($this->guestHeaders())
            ->postJson('/api/v1/passport/auth/register', [
                'email' => 'invitee-custom@example.com',
                'password' => 'password123',
                'invite_code' => $inviteCode->code,
            ]);
        ...
        $invitee = User::where('email', 'invitee-custom@example.com')->first();
        $this->assertNotNull($invitee);
        $this->assertSame($trialPlan->id, $invitee->plan_id);
        $this->assertSame($trialPlan->group_id, $invitee->group_id);
        $this->assertSame($trialPlan->device_limit, $invitee->device_limit);
        $this->assertSame($trialPlan->speed_limit, $invitee->speed_limit);
        $this->assertSame(25 * 1073741824, (int) $invitee->transfer_enable);
        $this->assertGreaterThanOrEqual($beforeRegister + (72 * 3600) - 5, (int) $invitee->expired_at);
    ...
    public function test_guest_invite_preview_returns_campaign_when_code_hits_active_campaign()
    {
        ...
        $this->getJson('/api/v1/guest/invite/preview?code=' . $inviteCode->code)
            ->assertOk()
            ->assertJsonPath('data.code', $inviteCode->code)
            ->assertJsonPath('data.type', 'campaign')
            ->assertJsonPath('data.gift_transfer_gb', 20)
            ->assertJsonPath('data.gift_hours', 72)
            ->assertJsonPath('data.countdown_expired_at', $campaign->expired_at);
    }

    public function test_guest_invite_preview_returns_normal_with_default_try_out_when_campaign_is_not_active()
    {
        $inviter = $this->createUser();
        $inviteCode = $this->createInviteCode($inviter, [
            'code' => 'PREVIEW002',
        ...
        $campaign = $this->createCampaign($inviter, $campaignPlan, [
        ...
        $this->getJson('/api/v1/guest/invite/preview?code=' . $inviteCode->code)
            ...
            ->assertJsonPath('data.gift_transfer_gb', 10)
            ->assertJsonPath('data.gift_hours', 24)
            ->assertJsonPath('data.countdown_expired_at', null);

        $campaign->refresh();
        $this->assertSame(InviteCampaign::STATUS_EXPIRED, $campaign->status);
    }

    public function test_guest_invite_preview_returns_invalid_for_unknown_code()
    {
        $this->getJson('/api/v1/guest/invite/preview?code=UNKNOWN001')
            ->assertOk()
            ->assertJsonPath('data.code', 'UNKNOWN001')
            ->assertJsonPath('data.type', 'invalid')
            ->assertJsonPath('data.gift_transfer_gb', 0)
            ->assertJsonPath('data.gift_hours', 0)
            ->assertJsonPath('data.countdown_expired_at', null);
    }
    ...
    public function test_admin_config_fetch_includes_invite_campaign_enable()
    {
        $admin = $this->createUser([
        ...
        $this->withHeaders($this->adminHeaders($admin))
            ->getJson($this->adminApiPath('/config/fetch?key=invite'))
            ->assertOk()
