...
const configLabels: Record<string, ConfigMeta> = {
  ...
  subscribe_token_in_path: { title: '订阅Token路径模式', description: '开启后订阅链接使用/订阅路径/token，关闭则保持?token=token。', switch: true },
...
function isSwitchField(fieldKey: string, value: any) {
  if (configLabels[fieldKey]?.switch) return true;
  ...
  const runTelegramAction = async (action: 'botInfo' | 'webhook' | 'test' | 'unbind') => {
    ...
    try {
      ...
      if (action === 'botInfo') {
        ...
        await apiPost('/config/save', normalizeConfig(values), { keepEmpty: true });
        await apiPost('/config/setTelegramWebhook', {
          telegram_bot_token: token,
          telegram_webhook_url: values.telegram_webhook_url,
          app_url: values.app_url,
        }, { form: true, keepEmpty: true });
